...
if __name__ == "__main__":
    ...
    '''
    #2021/01/17 remove solvent number > 2, reagent number > 3
    '''
    
    indexNames = data[ [CheckSemicolon_count(str(data.iloc[i]['Solvent (Reaction Details)'])) for i in range(len(data))] ].index
    data.drop(indexNames, inplace=True)
    data = data.reset_index(drop=True)
    indexNames = data[ [CheckSemicolon_count(str(data.iloc[i]['Reagent']), count = 3) for i in range(len(data))] ].index
    data.drop(indexNames, inplace=True)
    data = data.reset_index(drop=True)
    
    print('After remove the reaction with solvent number > 2 or reagent number > 3, data information: ')
    data.info()
    print('-'*50 + '\n')
    '''
